...
class LocalNotifManager: NSObject {
    ...
    let notification1 = "敷面膜"
    let notification2 = "睡觉"
    let notification3 = "喝水"
    ...
    func setInnerNotification(){
        ...
        for info in selectAll()!{
            if info.title == notification1{
                j1 = true
            }
            if info.title == notification2{
                j2 = true
            }
            if info.title == notification3{
                j3 = true
            }
        }
        if(!j1){
            setLocalNotification(aimDate: currentDate, with: "23:00", times: 1, title: notification1, body: "")
        }
        if(!j1){
            setLocalNotification(aimDate: currentDate, with: "23:00", times: 1, title: notification1, body: "")
        }
